...
PoolManager.Initialize.handler(async ({ event, context }) => {
  ...
  const feeNum = Number(event.params.fee);
  const DYNAMIC_FEE_FLAG = 0x800000; // 8388608
  const isDynamicFee = (feeNum & DYNAMIC_FEE_FLAG) !== 0;
  
  const feeDisplay = isDynamicFee ? "Dynamic" : `${(feeNum / 10000).toFixed(2)}%`;
  const poolName = `${token0.symbol} / ${token1.symbol} - ${feeDisplay}`;
