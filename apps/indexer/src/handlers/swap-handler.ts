...
PoolManager.Swap.handler(async ({ event, context }) => {
  ...
  // Calculate fees using the appropriate fee value
  // For dynamic fee pools: use the actual fee from the swap event
  // For vanilla pools: event.params.fee might be 0, so we check and use pool.feeTier if needed
  const DYNAMIC_FEE_FLAG = 0x800000;
  const isDynamicFeePool = (Number(pool.feeTier) & DYNAMIC_FEE_FLAG) !== 0;
  
  // Determine the fee to use for calculations
  let feeToUse: string;
  if (isDynamicFeePool) {
    // For dynamic fee pools, always use the fee from the event
    feeToUse = event.params.fee.toString();
  } else {
    // For vanilla pools, use event fee if it's non-zero, otherwise use pool.feeTier
    feeToUse = event.params.fee > 0n ? event.params.fee.toString() : pool.feeTier.toString();
  }
  ...
  const feesUSDUntracked = amountTotalUSDUntracked.times(
    new BigDecimal(feeToUse).div(new BigDecimal("1000000"))
  );
